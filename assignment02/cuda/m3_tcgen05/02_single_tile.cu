...
int main(int argc, char** argv) {
    ...
    bool use_proxy_fence = argc <= 2 || atoi(argv[2]) != 0;
    ...
    tcgen05_tile<<<1, 128>>>(dA, dB, dD, use_proxy_fence);
    CUDA_CHECK_KERNEL();
    ...
    fprintf(stderr, "proxy_fence=%s\n", use_proxy_fence ? "on" : "off");
