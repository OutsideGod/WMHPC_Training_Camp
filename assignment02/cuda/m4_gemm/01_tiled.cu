...
__global__ void gemm_tiled(const __nv_bfloat16* gA, const __nv_bfloat16* gB,
    ...
    uint32_t empty_addr = (uint32_t)__cvta_generic_to_shared(&empty);
    ...
    for (int it = 0; it < iters; ++it) {
        ...
        for (int i = tid; i < BM * BK; i += blockDim.x) {
            int row = i / BK, k = i % BK;
            *reinterpret_cast<__nv_bfloat16*>(&sA[swz128(row, k * 2)]) =
                gA[(size_t)(tileM + row) * K + tileK + k];
        }
        for (int i = tid; i < BN * BK; i += blockDim.x) {
            int row = i / BK, k = i % BK;
            *reinterpret_cast<__nv_bfloat16*>(&sB[swz128(row, k * 2)]) =
                gB[(size_t)(tileN + row) * K + tileK + k];
        }
